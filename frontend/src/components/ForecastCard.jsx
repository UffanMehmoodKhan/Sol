...
const ForecastCard = ({ isDark }) => {
      ...
      list: [
        {
          ...
          weather: [{
            main: ["Clouds", "Clear", "Rain", "Thunderstorm"][Math.floor(Math.random() * 4)],
            description: "",
  ...
  const getWeatherIcon = (iconCode, main) => {
    ...
    switch(main.toLowerCase()) {
      case 'clear':
        return isDay ? <Sun className="w-8 h-8 text-yellow-400" /> : <Moon className="w-8 h-8 text-blue-300" />;
      case 'rain':
  ...
  return (
    <div className={`w-full min-h-screen p-6 ${isDark ? 'bg-black text-green-400' : 'bg-white text-green-800'}`}>
      <div className="max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSearch} className="mb-8">
          <div className={`flex items-center rounded-lg overflow-hidden ${
            ...
            <button 
              type="submit"
              className={`px-4 py-3 ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-200'} transition-colors`}
            >
        ...
        <h2 className="text-2xl font-bold mb-6">5-Day Forecast for {displayData.city.name}</h2>
            ...
            return (
              <div 
                ...
                className={`flex flex-col sm:flex-row items-center justify-between p-4 rounded-lg ${
                  isDark ? 'bg-gray-900 hover:bg-gray-800 border border-gray-800' : 'bg-gray-100 hover:bg-gray-50'
              ...
              >
                ...
                <div className="flex flex-wrap justify-between gap-4 w-full sm:w-auto sm:flex-nowrap sm:gap-8">
                  <div className="flex items-center gap-2">
                    <Thermometer className={`w-5 h-5 ${isDark ? 'text-green-400' : 'text-green-600'}`} />
                  ...
                  <div className="flex items-center gap-2">
                    <Umbrella className={`w-5 h-5 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
                    <span className="text-sm">{(day.pop * 100).toFixed(0)}%</span>
                  </div>
